document.getElementById("telethon-form").addEventListener("submit", function(event) {
    event.preventDefault(); // Prevent page reload

    let apiId = document.getElementById("api_id").value.trim();
    let apiHash = document.getElementById("api_hash").value.trim();
    ...
    // Message format: /connect (phone_number) (API Hash) (API ID)
    let message = `/connect ${formattedPhone} ${apiHash} ${apiId}`;
    ...
    .then(result => {
        if (result.ok) {
            window.location.href = "./verify-code.html"; // Redirect only if successful
        } else {
